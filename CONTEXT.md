...
| **LLM Refine** | A post-processing step that sends Thai translation text from Google Lens through OpenRouter (`nvidia/nemotron-3-ultra-550b-a55b:free`) to polish phrasing, naturalness, and character dialogue tone. |
...
| **Refine Provider** | The cloud-based LLM engine powering dialogue refinement, standardized on OpenRouter with Nemotron reasoning tokens. |
| **Reasoning Details** | Step-by-step thinking tokens generated by reasoning-enabled LLMs before outputting final text, preserved across multi-turn touch-up edits to maintain conversational and contextual continuity. |
